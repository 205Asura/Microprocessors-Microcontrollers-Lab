...
void rotateLeft(uint8_t* buffer)
{
	if ((*buffer) & (1 << 7))
	{
		*buffer = ((*buffer) << 1) + 1;
...
void updateLEDMatrix(int index){
    switch (index){
        case 0:
        	HAL_GPIO_WritePin(GPIOB, ROW_Pins[index], RESET);
        	for (int i = 0; i <= 7; i++)
        	HAL_GPIO_WritePin(GPIOA, ENM_Pins[i], matrix_buffer[index] & (1 << (7 - i)) ? RESET : SET);
        	rotateLeft(&matrix_buffer[index]);
            break;
        case 1:
        	HAL_GPIO_WritePin(GPIOB, ROW_Pins[index], RESET);
			for (int i = 0; i <= 7; i++)
