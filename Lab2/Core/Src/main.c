...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	...
	if (counter1 <= 0)
	{
		...
		HAL_GPIO_WritePin(GPIOA, EN0_Pin, en[0] ? RESET : SET);
		HAL_GPIO_WritePin(GPIOA, EN1_Pin, en[1] ? RESET : SET);
		HAL_GPIO_WritePin(GPIOA, EN2_Pin, en[2] ? RESET : SET);
		HAL_GPIO_WritePin(GPIOA, EN3_Pin, en[3] ? RESET : SET);
		display7Seg(num);
		if (num == 1)
		{
			num = 2;
